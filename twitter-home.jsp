...
int userId = -1, count = -1, following = -1, followers = -1;
ResultSet rs = null;
...
try {
	...
	stmt = con.prepareStatement("SELECT (SELECT COUNT(*) FROM tweets WHERE uid = ?) AS count, "
						+ "(SELECT COUNT(*) FROM follows WHERE follower = ?) AS follows, "
						+ "(SELECT COUNT(*) FROM follows WHERE followee = ?) AS followers, "
						+ "(SELECT displayname FROM users WHERE uid = ?) AS displayname");
	...
	if(rs.next()) {
		...
		stmt = con.prepareStatement("(SELECT tweets.tid, tweets.uid, tweets.content, tweets.created, users.username, users.displayname FROM tweets "
					+ "JOIN users ON users.uid = tweets.uid WHERE users.uid = ?) UNION ALL"
					+ "(SELECT tweets.tid, tweets.uid, tweets.content, tweets.created, users.username, users.displayname FROM tweets "
					+ "JOIN users ON users.uid = tweets.uid JOIN follows ON follows.followee = tweets.uid WHERE follows.follower = ?) "
					+ "ORDER BY created DESC");
		stmt.setInt(1, userId);
		stmt.setInt(2, userId);
		rs = stmt.executeQuery();
	}
} catch (NumberFormatException nfe) { } //not expecting any other errors under normal conditions

String loginName = null;
int loginId = -1;
try {
	stmt = con.prepareStatement("SELECT displayname FROM users WHERE uid = ?");
	loginId = (Integer) session.getAttribute("user_id");
        ...
        <div class="row">
            ...
            <div class="span4" id="secondary">
                <div class="module mini-profile">
                    <div class="content">
                        <div class="account-group">
                            <a href="#">
                                ...
                                <b class="fullname"><%= userDisplayName %></b>
                    ...
                    <% if(loginId == userId) { %>
                    <form method="POST" action="twitter-compose.jsp">
                        <input type="text" autocomplete="off" name="content" class="tweet-box" placeholder="Compose new Tweet..." id="tweet-box-mini-home-profile">
			...
			<div class="js-stream-item stream-item expanding-string-item">
				...
				<div class="content">
					...
					<%= parseContent(clean(content), con) %>
				    </p>
				</div>
			    </a>
